...
enum HJFastKind: Int, Comparable {
    case none = 0
    case advisory = 1
    case voluntary = 2
    case obligatory = 3
    ...
    static func < (lhs: HJFastKind, rhs: HJFastKind) -> Bool { lhs.rawValue < rhs.rawValue }
    ...
    var tint: Color {
        ...
        case .advisory: return HJTheme.indigo
        case .voluntary: return HJTheme.verdant
        case .obligatory: return HJTheme.ink
        case .notPermitted: return HJTheme.clay
        }
    }
}

struct HJFastNote: Identifiable {
    let id: String
    let label: String
    let kind: HJFastKind
    let detail: String
...
struct HJDayFasting {
    let jdn: Int
    let hijri: HJHijriDate
    let weekday: Int
    let notes: [HJFastNote]
...
enum HJFasting {
    ...
    static func status(forJDN jdn: Int, store: HJStore) -> HJDayFasting {
        let hijri = store.hijri(forJDN: jdn)
        let weekday = HJCalendar.weekdayIndex(jdn: jdn)
        var notes: [HJFastNote] = []

        // --- obligatory
        if hijri.month == 9 {
            notes.append(HJFastNote(id: "ramadan",
                                    label: "Ramadan",
                                    kind: .obligatory,
        ...
        if hijri.month == 10 && hijri.day == 1 {
            notes.append(HJFastNote(id: "eidfitr",
                                    label: "Eid al-Fitr",
                                    kind: .notPermitted,
        ...
        if hijri.month == 1 && hijri.day == 9 {
            notes.append(HJFastNote(id: "tasua",
        ...
        if hijri.month == 1 && hijri.day == 10 {
            notes.append(HJFastNote(id: "ashura",
        ...
        if hijri.month == 12 && hijri.day >= 1 && hijri.day <= 9 {
            notes.append(HJFastNote(id: "firstten",
        ...
        if hijri.month == 12 && hijri.day == 9 {
            notes.append(HJFastNote(id: "arafah",
        ...
        if hijri.month == 10 && hijri.day >= 2 {
            notes.append(HJFastNote(id: "shawwal6",
        ...
        if hijri.day >= 13 && hijri.day <= 15 {
            ...
            notes.append(HJFastNote(id: "whiteday",
        ...
        if hijri.month == 8 && hijri.day <= 28 {
            notes.append(HJFastNote(id: "shaban",
                                    label: "Sha'ban",
                                    kind: .advisory,
        ...
        if hijri.month == 8 && hijri.day == 30 {
            notes.append(HJFastNote(id: "doubt",
        ...
        if weekday == 1 || weekday == 4 {
            notes.append(HJFastNote(id: "monthu",
        ...
        if weekday == 5 && !hasOtherVoluntary {
            notes.append(HJFastNote(id: "friday",
        ...
        if weekday == 6 && !hasOtherVoluntary {
            notes.append(HJFastNote(id: "saturday",
        ...
        return HJDayFasting(jdn: jdn, hijri: hijri, weekday: weekday, notes: ordered)
    }
    ...
    /// day's own sheet.
    static func month(year: Int, month: Int, store: HJStore) -> [HJDayFasting] {
        let length = HJCalendar.hijriMonthLength(year: year, month: month)
        var out: [HJDayFasting] = []
    ...
    static func restrictedDays(year: Int, store: HJStore) -> [HJDayFasting] {
        var out: [HJDayFasting] = []
