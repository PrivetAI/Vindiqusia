...
struct VQCalendarPageShape: Shape {
    var rows: Int = 3
    var columns: Int = 4
...
struct VQDoubleChevronShape: Shape {
    var direction: Int = 0
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.addPath(VQChevronShape(direction: direction).path(in: left))
        path.addPath(VQChevronShape(direction: direction).path(in: right))
        return path
    }
}
...
struct VQTodayGlyphShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
...
struct VQBandOrnamentShape: Shape {
    var cells: Int = 8
