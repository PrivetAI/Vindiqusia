...
/// `day` 1...30.
struct VQHijriDate: Equatable, Hashable, Codable {
    var year: Int
    var month: Int
    var day: Int
...
struct VQCivilDate: Equatable, Hashable, Codable {
    var year: Int
    var month: Int
    var day: Int
...
enum VQCalendar {
    ...
    static func hijriDayOfYear(_ date: VQHijriDate) -> Int {
        hijriDaysBeforeMonth(date.month) + date.day
    }

    // MARK: Hijri <-> JDN

    static func jdn(fromHijri date: VQHijriDate) -> Int {
        hijriEpochJDN + hijriDaysBeforeYear(date.year) + hijriDaysBeforeMonth(date.month) + date.day - 1
    }

    static func hijri(fromJDN jdn: Int) -> VQHijriDate {
        ...
        return VQHijriDate(year: year, month: month, day: day)
    }
    ...
    static func civilDayOfYear(_ date: VQCivilDate) -> Int {
        var total = 0
        var m = 1
        while m < date.month {
    ...
    static func jdn(fromCivil date: VQCivilDate) -> Int {
    ...
    static func civil(fromJDN jdn: Int) -> VQCivilDate {
        let a = jdn + 32044
        ...
        return VQCivilDate(year: year, month: month, day: day)
    }
    ...
    static func clampHijri(_ date: VQHijriDate) -> VQHijriDate {
    ...
    static func clampCivil(_ date: VQCivilDate) -> VQCivilDate {
    ...
    /// of the arithmetic.
    static var minJDN: Int { jdn(fromHijri: VQHijriDate(year: minHijriYear, month: 1, day: 1)) }
    static var maxJDN: Int { jdn(fromHijri: VQHijriDate(year: maxHijriYear, month: 12, day: hijriMonthLength(year: maxHijriYear, month: 12))) }
    ...
    static func civilDate(from date: Date, timeZone: TimeZone = .current) -> VQCivilDate {
        let parts = civilReference(timeZone: timeZone).dateComponents([.year, .month, .day], from: date)
        return VQCivilDate(year: parts.year ?? 2026, month: parts.month ?? 1, day: parts.day ?? 1)
    }
...
enum VQNames {
    ...
    static func weekday(_ index: Int, arabic: Bool) -> String {
        let i = VQCalendar.floorMod(index, 7)
...
struct VQFormatter {
    ...
    func hijriMonthName(_ month: Int) -> String {
        VQNames.hijriMonth(month, marked: markedSpelling)
    }

    func weekdayName(_ index: Int) -> String {
        VQNames.weekday(index, arabic: arabicWeekdays)
    }

    func weekdayShortName(_ index: Int) -> String {
        VQNames.weekdayShort(index, arabic: arabicWeekdays)
    }

    /// "12 Ramadan 1447 AH"
    func hijriLong(_ date: VQHijriDate) -> String {
        "\(date.day) \(hijriMonthName(date.month)) \(date.year) AH"
    }

    /// "12 Ramadan"
    func hijriDayMonth(_ date: VQHijriDate) -> String {
        "\(date.day) \(hijriMonthName(date.month))"
    }

    /// "Ramadan 1447 AH"
    func hijriMonthYear(_ date: VQHijriDate) -> String {
        "\(hijriMonthName(date.month)) \(date.year) AH"
    }

    /// "12 March 2026 CE"
    func civilLong(_ date: VQCivilDate) -> String {
        "\(date.day) \(VQNames.civilMonth(date.month)) \(date.year) CE"
    }

    /// "12 Mar 2026"
    func civilShort(_ date: VQCivilDate) -> String {
        "\(date.day) \(VQNames.civilMonthShort(date.month)) \(date.year)"
    }

    /// "March 2026"
    func civilMonthYear(_ date: VQCivilDate) -> String {
        "\(VQNames.civilMonth(date.month)) \(date.year)"
    }
